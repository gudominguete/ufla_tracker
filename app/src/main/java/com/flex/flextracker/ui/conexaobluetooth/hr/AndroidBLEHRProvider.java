    ...
    private final BluetoothGattCallback btGattCallbacks = new BluetoothGattCallback() {
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
            ...
            try {
                ...
                Integer val = Integer.parseInt(String.valueOf(arg0.getValue()[0]));
...
//                }
    ...
    public void disconnect() {
        ...
        if (isConnected) {
            ...
            Log.i("teste","close btGatt here in disconnect()");
            BluetoothGatt copy = btGatt;
            if (copy != null)
                copy.close();
            btGatt = null;
    ...
    private void reportDisconnectFailed(String string) {
        Log.i("teste","disconnect failed: " + string);
    ...
    private synchronized boolean checkBtGatt(BluetoothGatt gatt, boolean onlyLogError) {
        if (btGatt == null) {
            if (!onlyLogError)
                Log.i("teste","checkBtGatt, btGatt == null => true");
            btGatt = gatt;
            return true;
        }
        if (btGatt == gatt) {
            if (!onlyLogError)
                Log.i("teste","checkBtGatt, btGatt == gatt => true");
            return true;
        }
        Log.i("teste","checkBtGatt, btGatt(" + btGatt + ") != gatt(" + gatt + ") => false");
        return false;
    }
}
